...
class PositionManager:
    ...
    async def sync_positions_with_api(self):
        ...
        for api_pos in api_positions:
            ...
            # Normalize API outcome labels (e.g., team names) to canonical yes/no
            # when possible so local keys remain consistent.
            if outcome not in ("yes", "no"):
                normalized = await self.api_client.normalize_outcome_to_yes_no(market_slug, raw_outcome)
                if normalized:
                    outcome = normalized
        ...
        for key, local_pos in self.positions.items():
            if key not in api_lookup:
                logger.warning(
                    f"Position closed externally: {local_pos['market_slug']} | "
                    f"{local_pos['outcome']} (not found in API)"
                )
        ...
        for key, local_pos in self.positions.items():
            if key in api_lookup:
                api_shares = api_lookup[key]["shares"]
                ...
                if abs(api_shares - local_shares) > 0.01:
                    ...
                    if api_shares == 0:
                        ...
                        self.update_position_shares(
                            ...
                            local_pos["outcome"],
                            api_shares,
                            new_invested=api_lookup[key].get("invested"),
                        )
